...
def pinn_loss(xb, yb):
    ...
    res, q = model.pde_residual_total(xb, eps=eps)


    # --- unnormalize pt_ref, Bz (needed for mask + omega scale) ---
    ptref_norm = x_f[:, ptref_index]
    pt_ref = ptref_norm * x_std_t[ptref_index] + x_mean_t[ptref_index]

    bz_norm = x_f[:, bSZ_index]
    Bz = bz_norm * x_std_t[bSZ_index] + x_mean_t[bSZ_index]

    # --- masks ---
    mask_q  = (q.abs() >= 0.5)
    mask_pt = (pt_ref > 0.0)          # sentinel/invalid pt_ref (e.g., -1) dışarı
    mask = mask_q & mask_pt
    mask_frac = mask.float().mean()

    # --- omega scale (same form as in pde_residual) ---
    pt_ref_c = torch.clamp(pt_ref.abs(), min=1e-2)
    omega = model.kappa * q * Bz / (pt_ref_c + eps)

    omega_scale = torch.sqrt((omega**2).mean()).detach() + 1e-3
    res_scaled = res / omega_scale

    # --- masked PDE loss (scaled) ---
    if mask.any():
    ...
    if DEBUG_PDE and (not DEBUG_ONCE["done"]):
        with torch.no_grad():
            ...
            print("omega abs mean/max:",
                  float(omega.abs().mean().item()),
                  float(omega.abs().max().item()))

            print("omega_scale:", float(omega_scale.item()))
            print("res abs mean/max (all):",
                  float(res.abs().mean().item()),
                  float(res.abs().max().item()))
